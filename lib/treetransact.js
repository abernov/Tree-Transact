...
module.exports = function (treeSchemaDef, childSchemaDef) {
  ...
  function cmpIds(id1, id2) {
    if (id1 && typeof id1 === 'object') return id1.equals(id2);
    return (id1 === id2);
  }
...
//return this;
  ...
  TreeTransact.statics.getChildren = async function (doc, returnDoc) {
    var ch;
    var repaired = {};
    var checked = {};
    var res;
    ...
    var checkChild = async function (ch, childIdx) {
      try {
        var r = await self.childDocExistsCallback(ch, docId);
        checked[ch._id] = r;
  ...
  TreeTransact.statics.addChild = async function (doc, index, childObject, checkChildDoc) {
    ...
    while (true) {
      ...
      if (idx < 0) {
        ...
        if (this.onBeforeAddChild) childObject = this.onBeforeAddChild(childObject, idx, doc.children);
  ...
  try {
    if (cmpIds(childId, destId)) throw new TreeError('MoveToSelf', srcId, childId);
    if (cmpIds(srcId, destId)) {
