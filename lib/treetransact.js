...
module.exports = function (treeSchemaDef, childSchemaDef) {
  ...
  TreeTransact.statics.getByIdAndCheck = async function (docId) {
    ...
    if (!doc) throw new Error('Document not exists: ' + docId);
    return doc;
  }
    ...
    while (true) {
      ...
      if (idx < 0 ) {
        if (newState == state_RECEIVED) {
          ...
          if (newState !== state_REMOVE_CHILD) throw new Error('Child not exists: ' + childId);
          return doc;
        }
      } else {
        tran = doc.children[idx].tran;
        if (tran) {
          if (!cmpIds(tran._id, tranId)) throw new Error('Child has invalid transaction _id. Child: ' + childId);
          if (tran.state === newState) return doc;
        }
        if (newState === state_RECEIVED && (!tran || tran.state !== state_RECEIVED)) throw new Error('Child already exists: ' + childId);
      }
      if (!newState) {
        doc.children[idx].tran = undefined;
      } else {
        if (newState === state_REMOVE_CHILD) {
          if (!tran) throw new Error('Transaction not exists: ' + childId);
          doc.children.splice(idx, 1);
        } else {
          if (tran) throw new Error('Transaction already exists: ' + childId);
...
//return this;      
  ...
  TreeTransact.statics.removeCheck = async function (doc, childIdx, res, newDocFun) {
    var check = doc.children[childIdx].check;
    try {
      if (check > Date.now()) {
        if (await this.childDocExistsCallback(ch)) {
          doc.children[childIdx].check = undefined;
        } else {
          doc.children.splice(childIdx, 1);
        }
        doc = await this.trySave(doc);
        if (!doc) doc = await this.getByIdAndCheck(docId);
        newDocFun(doc);
  ...
  TreeTransact.statics.addChild = async function (doc, index, childObject, checkChildDoc) {
    ...
    while (true) {
      ...
      if (idx < 0) {
        ...
        throw new Error('Child already exists: ' + childObject._id);
      }
    }
  }

  function throwChildNotExistsErr(childId) {
    throw new Error('Child not exists: ' + childId);
  } 
    ...
    while (true) {
      ...
      if (idx >= 0) {
        if (doc.children[idx].tran) {
          ...
          idx = doc.getChildIdx(childId);
          if (idx < 0) throwChildNotExistsErr(childId);
          if (doc.children[idx].tran) throw new Error('Document is busy: ' + docId);
        }
        ...
        throwChildNotExistsErr(childId);
      }
    }
  }
  ...
  TreeTransact.statics.getDocAndRepairChild = async function (docId, childId) {
    ...
    if (idx < 0) throw new Error('Child not found: ' + childId);
  ...
  TreeTransact.statics.setChildDocExistsCallback = function (childDocExists) {
    if (childDocExists && typeof childDocExists !== 'function') throw new Error('Invalid childDocExists function');
  ...
  try {
    if (childId.equals(destId)) throw new Error("Can't be moved to itself: " + childId);
